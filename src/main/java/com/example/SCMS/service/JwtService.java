...
public class JwtService {
    ...
    public String generateToken(Map<String, Object> extraClaims, String fullName) {
        return buildToken(extraClaims, fullName, jwtExpiration);
    }

    public boolean isTokenValid(String token, String fullName) {
        final String extractedFullName = extractFullName(token);
        return (extractedFullName.equals(fullName)) && !isTokenExpired(token);
    }

    private String buildToken(Map<String, Object> extraClaims, String fullName, long expiration) {
        return Jwts
                .builder()
                .setClaims(extraClaims)
                .setSubject(fullName) // 🔹 Set Full Name as Subject
                .setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + expiration))
                .signWith(getSignInKey(), SignatureAlgorithm.HS256)
                .compact();
